# Rabbit Pie 単4×3 + ON/OFFスイッチ内蔵ケース（一体型）

[Rabbit Pie](https://okikata.org/rp/index.html) の背面ケース
(`case_fix_2026.stl`) を改造し、下端を延長して **単4電池3本（直列 4.5V）と
ON/OFFスライドスイッチをそのまま搭載できる電池バーを一体化**したケースです。
これまでマジックテープで貼っていた、スイッチ付き単4×3本の電池ボックスは
不要になります。
...
- **印刷方法は従来と同一** — 電池バーの背面はケース背面と同一平面。
  ...
  （造形高さ 13.3mm）
- **持ちやすい** — Game Boy 風の縦長シルエット（59.6×125.5mm）。
...
| ファイル | 説明 |
|---|---|
| `stl/case_battery_3AAA_switch.stl` | **印刷するのはこれ1個**（59.6 × 125.5 × 13.3mm） |
...
2. **スイッチを差す** — 左タワー上部のポケットに外側からスイッチ本体を
   ...
   開口からそのまま操作できる
3. **配線** — ch1（x- 側、外部リード線）からの配線をスイッチへ、
   スイッチのもう一方の端子から配線レースウェイ（タワー内の連続した溝）
   を通してケース底壁の**貫通トンネル（左）**からケース内部へ。
   ch3（x+ 側、外部リード線）はレースウェイを通して**貫通トンネル（右）**
   から直接ケース内部へ。これまで電池ボックスを繋いでいた電源入力へ接続する
4. **電池を入れる** — 上（前面側）からパチンと押し込むだけ。ジグザグ配線
...
| 電池バー | 総高 125.5mm（ケース本体69.3mm＋バー延長分）、前面はリム+0.6mm |
...
- スイッチ本体の外形ブロックとケースの干渉ゼロ（ポケット内に収まる）
