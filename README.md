# Rabbit Pie 単4×3 電池内蔵ケース（一体型、3バリエーション）
...
**3つのバリエーション**を用意しています。用途に応じて選んでください。

| バリエーション | ファイル | 内容 |
|---|---|---|
| **BASIC** | `stl/case_3AAA_basic.stl` | 電池3本 + ON/OFF電源スイッチのみ（59.6×125.5×13.3mm） |
| **BASIC_OPTIMIZED** | `stl/case_3AAA_basic_optimized.stl` | BASICと同機能だが、タワー内部の配線レースウェイ（長い空洞）を必要最小限の細い配線ダクトに置き換え、内部空洞を減らして印刷を最適化したもの |
| **SPEAKER** | `stl/case_3AAA_speaker.stl` + `stl/speaker_lid.stl` | 電池3本 + 電源スイッチ + ミュートスイッチ + スピーカー（着脱式リッド、別部品）。（59.6×130.0×13.3mm） |

![preview](docs/preview.png)

BASIC・BASIC_OPTIMIZED は追加の印刷部品ゼロ（蓋なし・リベットなし・
ネジなし）。SPEAKER はスピーカー用の着脱式リッドのみ追加印刷が必要です
（スピーカーを本体に恒久的にフリクションフィットさせるのではなく、
着脱できるようにするため。詳細は下記「スピーカーの取り付け」参照）。

### BASIC と BASIC_OPTIMIZED の違い

BASIC は左右タワー内部に、ケース接合部から最終チャンネル手前まで連続した
「配線レースウェイ」（幅2mm×高さ約11.7mm×長さ数十mmの空洞）を持たせて
いますが、実際に配線が通る部分はごく一部です。BASIC_OPTIMIZED は、この
連続した空洞を埋め、電極まわりの必要な範囲だけに高さ3mmの細い配線ダクトを
通す設計に変更しています。内部の大きな空洞（＝印刷時にタワー上部が
ブリッジ／庇状になる部分）が減るため、印刷の安定性や速度がやや向上します
（体積は電池3個分でおよそ+0.7cm³＝+2.6%増えますが、内部空洞は大幅に
減少）。配線経路・電極スロットの導通はどちらも同一設計・同一検証済みです。
...
| `stl/case_3AAA_basic_optimized.stl` | BASIC_OPTIMIZED: 印刷するのはこれ1個 |
...
| `generate_battery_case.py` | パラメトリック生成スクリプト（全バリエーション+リッドを一括生成） |
...
- 姿勢: 背面（平らな面）を下。**従来のケースと同じ**。リッドも印刷面を下に
  したまま出力される向きで生成済み
- ノズル: **0.4mm推奨**。スナップ開口の絞り（`SNAP_PINCH`=0.3mm）や電極
  スロットの隙間（`SLOT_T`=1.0mm）がノズル径ギリギリ〜以下の寸法のため、
  0.8mmノズルでは開口が潰れたり電極が導通しないリスクが高い
- レイヤー 0.2mm、壁 3周、インフィル 15%、サポート不要（全部品）
- 電池がきつい/ゆるい場合は `CELL_FIT`（チャンネル径の余裕）と
  `SNAP_PINCH`（開口の絞り量）を調整して再生成

### 高速印刷したい場合（例: Bambu Lab P1S）

このパーツは高さ13.3mmで壁主体・インフィル少なめの薄型形状なので、
0.4mmノズルのまま以下を調整すると印刷時間を大きく短縮できる:

- レイヤー高さを 0.2mm → **0.28〜0.3mm** に上げる（層数が7割程度に減る）
- 壁は3周のまま維持（スナップ部・電極スロット周りの強度に必要なため
  減らさない方が安全）
- インフィルは 15%→10% グリッドで十分（機能的には壁が主構造）
- 外壁・スパースインフィルの速度を上げた高速プロファイルを使用
- サポート・アダプティブレイヤーはOFF（元々サポート不要設計）

内部の空洞を減らして印刷をさらに安定・高速化したい場合は
`case_3AAA_basic_optimized.stl`（BASIC_OPTIMIZED）を使うとよい。
...
実行すると `stl/case_3AAA_basic.stl`・`stl/case_3AAA_basic_optimized.stl`・
`stl/case_3AAA_speaker.stl`・`stl/speaker_lid.stl` が一括生成されます。
...
| 配線トンネル | 左 x −19〜−16 ／ 右 x 16〜19（全バリエーション共通）、z −2.8〜−0.6 |
| 配線ダクト（BASIC_OPTIMIZEDのみ） | 高さ3mmの細いダクトのみ（`THIN_DUCT_Z`）。通常版は高さ約11.7mmの連続レースウェイ |
...
- 装着状態の単4セル3本（Φ10.5×44.5）はケースと干渉ゼロ（全バリエーション）
...
- 配線トンネル・ダクトは連続して開通していることを確認
  （BASIC_OPTIMIZEDの細い配線ダクトでも、ch1→電源スイッチ→左トンネル、
  ch3→右トンネルの経路が開通していることを個別に確認）
- **電極スロット6箇所すべて**（左右タワー×ch1/ch2/ch3）が、各チャンネルの
  ...
  修正して解消）
- **右トンネル欠落バグを修正**: BASICでは ch3（x+ タワーの外部リード線）が
  ケース内部へ抜けるトンネルが存在せず、配線経路が行き止まりになっていた。
  電源スイッチ・スピーカーの有無にかかわらず左右トンネルを常設するよう修正
